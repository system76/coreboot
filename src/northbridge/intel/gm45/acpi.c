...
static unsigned long acpi_fill_dmar(unsigned long current)
{
	...
	current += acpi_create_dmar_drhd_4k(current, 0, 0, IOMMU_BASE_VC1);
	current += acpi_create_dmar_ds_pci(current, 0, 0x1b, 0);
	acpi_dmar_drhd_fixup(tmp, current);

	if (stepping != STEPPING_B2 && igd_active) {
		tmp = current;
		current += acpi_create_dmar_drhd_4k(current, 0, 0, IOMMU_BASE_GFX);
	...
	if (me_active) {
		tmp = current;
		current += acpi_create_dmar_drhd_4k(current, 0, 0, IOMMU_BASE_ME);
	...
	current += acpi_create_dmar_drhd_4k(current, DRHD_INCLUDE_PCI_ALL, 0, IOMMU_BASE_VC0);
