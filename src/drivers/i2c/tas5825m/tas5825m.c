...
static int tas5825m_write_at(struct device *dev, uint8_t addr, uint8_t value) {
	return smbus_write_byte(dev, addr, value);
}

//TODO: use I2C block write for better performance
static int tas5825m_write_block_at(struct device *dev, uint8_t addr, const uint8_t * values, uint8_t length) {
	int res = 0;
...
static int tas5825m_set_page(struct device *dev, uint8_t page) {
	return tas5825m_write_at(dev, 0x00, page);
}

static int tas5825m_set_book(struct device *dev, uint8_t book) {
