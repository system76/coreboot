 ...
 * PCIe, USB3 and DisplayPort connectivity than RV(1) or PCO. A Matisse SoC is always PCO
 * silicon, a Dali SoC can either be RV2 or fused-down PCO silicon that has the same
 * connectivity as the RV2 one and Pollock is always RV2 silicon. Matisse and Dali are in a FP5
...
static uint32_t get_internal_silicon_type(void)
{
	...
	size_t hob_size = 0;
	const struct matisse_misc_data *hob;

	if (silicon_type)
		return silicon_type;

#if CONFIG(PLATFORM_USES_FSP2_0)
	hob = fsp_find_extension_hob_by_guid(MATISSE_MISC_DATA_HOB_GUID.b, &hob_size);
#else
	hob = NULL;
	hob_size = 0;
#endif
	...
	if (hob == NULL || hob_size == 0) {
		printk(BIOS_ERR, "Couldn't find Matisse misc data HOB.\n");
		return 0;
	}

	if (hob->version != MATISSE_MISC_DATA_VERSION) {
		printk(BIOS_ERR, "Unexpected Matisse misc data HOB version.\n");
		return 0;
	}
...
static bool is_fam17_1x(void)
{
	...
	return cpuid_eax(1) >> 8 == MATISSE_B1_CPUID >> 8;
}
...
{
	/* only mask stepping */
	return cpuid_eax(1) >> 4 == MATISSE_B1_CPUID >> 4;
}
