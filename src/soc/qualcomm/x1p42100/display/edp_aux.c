...
#define EDP_AUX_INTERRUPT                                                                    \
	(AUX_INTR_I2C_DONE | AUX_INTR_WRONG_ADDR | AUX_INTR_CONSECUTIVE_TIMEOUT |            \
	 AUX_INTR_CONSECUTIVE_NACK_DEFER | AUX_INTR_WRONG_RD_DATA_CNT | AUX_INTR_DEFER_I2C | \
...
static int edp_msg_fifo_rx(void *buffer, size_t size)
{
	...
	/*	printk(BIOS_DEBUG, "AUX RX dummy  = 0x%08x\n", data);
	...
	for (i = 0; i < len; i++) {
		data = read32(&edp_auxclk->aux_data);
		//	printk(BIOS_DEBUG, "AUX RX word[%d] = 0x%08x\n", i, data);
		dp[i] = (u8)((data >> 8) & 0xff);
	}
...
ssize_t edp_aux_transfer(unsigned int address, u8 request, void *buffer, size_t size)
{
	...
	/* msg sanity check */
	if ((native && (size > AUX_CMD_NATIVE_MAX)) || (size > AUX_CMD_I2C_MAX)) {
	...
	printk(BIOS_INFO, "DP AUX %s done: status=%d type=%u addr=0x%06x %s=%zu %s=%zd\n",
	       read ? "READ" : "WRITE", 0, /* status: 0 = success */
	       request, address, read ? "req_size" : "size", size,
	       read ? "bytes_read" : "bytes_written", ret);
	...
	if (buffer && size) {
		...
		printk(BIOS_INFO, ":");
		for (i = 0; i < size; i++)
			printk(BIOS_INFO, "%02x%s", ((u8 *)buffer)[i],
			       (i == size - 1) ? "]\n" : ":");
	}
