...
void mdss_layer_mixer_setup(struct edid *edid)
{
	...
	write32(&mdp_ctl_0->ctl_layer0,
		0x100002D); // VIG1:VIG0 > 0x5: Stage3 FG for layermixer 0
	...
	if (dual) {
		...
		write32(&lm1->layer_op_mode, 0x80000000); // SPLIT_LEFT_RIGHT
		...
		write32(&mdp_ctl_0->ctl_layer1,
			0x100002D); // VIG1:VIG0 > 0x5: Stage3 FG for layermixer 1
