...
#define  PCR_PSFX_TO_SHDW_PCIEN		0x1C

/* DMI3DP registers */
#define DMI3DP_DECODE_CTRL_IOC_MCHBAR_REG	0x7978
#define DMI3BAR_IOC_MCHBAR_REG			0x7938
#define DMI_DECODE_EN		0x1
...
static void pcd_die_early_ip_init(void)
{
	...
	/* Disable DMI Decode */
	clrbits32((void *)MCH_BASE_ADDRESS + DMI3DP_DECODE_CTRL_IOC_MCHBAR_REG, DMI_DECODE_EN);
	/* Disable DMI RCRB */
	clrbits32((void *)MCH_BASE_ADDRESS + DMI3BAR_IOC_MCHBAR_REG, DMI_DECODE_EN);
}
...
static void pcd_die_early_sa_init(void)
{
	const struct sa_mmio_descriptor soc_fixed_pci_resources[] = {
		...
		{ NOHOMBASE, NOHOM_BASE_ADDRESS, NOHOM_SIZE, "NOHOMBASE" },
		{ NOHOMLIMIT, NOHOM_LIMIT_ADDRESS, NOHOM_SIZE, "NOHOMLIMIT" },
	};
...
static void pcd_die_config_acpibase(void)
{
	...
	if (pmc_reg_value == 0xffffffff) {
		...
		pcr_rmw32(PID_PSF8, pmc_base_reg + PCR_PSFX_TO_SHDW_PCIEN,
